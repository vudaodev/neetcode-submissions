...
class Solution:
    def isSameTree(self, p: Optional[TreeNode], q: Optional[TreeNode]) -> bool:
        if not p and not q: # 2 Null values
            return True
        if p and not q: # q is Null but p isn't
            return False
        if q and not p: # p is Null but q isn't
            return False
        if p.val != q.val: # Return False since values are not equal
            return False
        # If False is ever returned, the trees are not the same
